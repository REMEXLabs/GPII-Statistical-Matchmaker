...
stat.match = function (requestProxy, payload) {
	// Rejection tests
	if (!("deviceReporter" in payload)) { fluid.log("Invalid SMM payload: 'deviceReporter' missing."); };
	if (!("solutions" in payload.deviceReporter)) { fluid.log("Invalid SMM payload: 'deviceReporter.solutions' missing."); };
	var solutions = payload.deviceReporter.solutions;
...
stat.fuseContextsByPreference = function (contextA, contectB) {
	var result = contextA;
	if (!("preferences" in result)) result.preferences = {};
	// Add contextB
	if ("preferences" in contectB) {
		fluid.each(contectB.preferences, function(appPrefs, appKey){
...
stat.filterPreferences = function (preferences, solutions) {
	...
	fluid.each(preferences, function(prefValues, prefKey) {
		...
		if (found) {
			result[prefKey] = {}
			result[prefKey].preferences = preferences[prefKey];
			result[prefKey].active = true;
		}
	});
	return result;
}
